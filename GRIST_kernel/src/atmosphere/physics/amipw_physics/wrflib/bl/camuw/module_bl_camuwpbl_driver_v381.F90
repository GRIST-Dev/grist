...
! AMIPW_PHYSICS is GRIST modification

module module_bl_camuwpbl_driver_v381
  ...
  use grist_mpi
  use module_cam_bl_diffusion_solver,   only : vdiff_selector
#else
...
#ifdef UW_CAM5
  real(r8), parameter  :: kv_freetrop_scale = 1.0      !! LiXH add, Eddy diffusivity scale factor for the free troposphere
#endif
CONTAINS
...
#endif
#ifdef AMIPW_PHYSICS
    use module_cam_bl_diffusion_solver !!, only : compute_vdiff, any, operator(.not.)
#else
...
#ifdef UW_CAM5
             if ((kv_freetrop_scale /= 1._r8) )then
                 kvh = kvh * kv_freetrop_scale
                 kvm = kvm * kv_freetrop_scale
                 kvq = kvq * kv_freetrop_scale
             end if
#endif
             !! ----------------------------------------------- !       
...
!-----------------------------------------------------------------------------------------
    ...
    use module_cam_bl_diffusion_solver,only : init_vdiff, vdiff_select
#ifndef AMIPW_PHYSICS
    ...
    if (mpi_rank().eq.0) then
       write(6,*)'Initializing vertical diffusion (vertical_diffusion_init)'
    end if
#else
    if (masterproc) then
    ...
    case ( 'diag_TKE' ) 
#ifdef AMIPW_PHYSICS
       if( mpi_rank().eq.0 ) write(iulog,*) 'vertical_diffusion_init: eddy_diffusivity scheme'
       if( mpi_rank().eq.0 ) write(iulog,*) 'UW Moist Turbulence Scheme by Bretherton and Park'
#else
...
#ifdef AMIPW_PHYSICS
       if( mpi_rank().eq.0 ) write(6,*) 'vertical_diffusion: nturb, ntop_eddy, nbot_eddy ', nturb, ntop_eddy, nbot_eddy
#else
...
#ifdef AMIPW_PHYSICS
       if (mpi_rank().eq.0) then
          write(6,*)'Using turbulent mountain stress module'
          write(6,*)'  tms_orocnst = ',tms_orocnst
       end if
#else
       if (masterproc) then
...
end module module_bl_camuwpbl_driver_v381
