...
#ifdef UW_CAM5
  real(r8),         parameter :: lbulk_max      = 40.e3_r8      ! LiXH add, Maximum master length scale in CAM5 PHYS 
  real(r8),         parameter :: leng_max       = 40.e3_r8      ! LiXH add, Maximum dissipation length scale in CAM5 PHYS
#endif
...
!#endif
    if( kind .ne. r8 ) then
#ifdef AMIPW_PHYSICS
        call endrun('wrong KIND of reals passed to init_diffusvity -- exiting.')
#else
  ...
  subroutine compute_eddy_diff( lchnk  ,                                                            &
    ...
    !-------------------------------------------------------------------- !
#ifdef AMIPW_PHYSICS
    use module_cam_bl_diffusion_solver, only: compute_vdiff
#else
...
#ifdef UW_CAM5
             leng(i,k) = min(leng_max, leng(i,k))
#endif
...
#ifdef UW_CAM5
                 leng(i,ks) = min(leng_max, leng(i,ks))
#endif
              end do
          end if
       end do ! k
...
#ifdef UW_CAM5
             leng_imsi = min(leng_max, leng_imsi)
#endif
