...
!
!   This is a modified Tiedtke-Bechtold version (with -DMOIST_CU) based on:
!
!   Li, X., W. Chu, Y. Zhang, and Y. Wang, (2024), Extending a dry-environment convection 
!   parameterization to couple with moist turbulence and a baseline evaluation in the GRIST model. 
!   Quarterly Journal of the Royal Meteorological Society, 150(763), 3368-3384.doi:https://doi.org/10.1002/qj.4763.
!###########################################################

module module_cu_mtiedtke_v381
...
#elif (defined MOIST_CU)
      parameter(cprcon = 1.2e-3)
#else
...
#ifdef MOIST_CU
                ndc, omega, cmfmc ,pblh                        )
#else
                ndc, omega, cmfmc                              ) ! added by yiz for grist
#endif
!-------------------------------------------------------------------
#ifdef MOIST_CU
    use grist_PhysW_nml_module, only: PhysW_bl_scheme
#endif
      implicit none
!-------------------------------------------------------------------
...
!-------------------------------------------------------------------
      ...
      real,    dimension(ims:ime, jms:jme) ::                           &
                                        ...
                                        hfx                            
#ifdef MOIST_CU
     real,    dimension(ims:ime, jms:jme) :: pblh
#endif
...
!--------------------------- local vars ------------------------------
      ...
      real     , dimension(its:ite) ::                  &
                                        ...
                                        heatflux                       
#ifdef MOIST_CU
      integer  , dimension(its:ite) ::  kinv
#endif
...
#ifdef MOIST_CU
      if(trim(PhysW_bl_scheme).eq. 'camuw')then
      do i=its,ite
        do k=kts,kte-1
          if((pblh(i,j)+2.-ghti(i,k))*(pblh(i,j)+2.-ghti(i,k+1)).le.0.)then
            kinv(i) = k
            go to 15
          end if
        end do
...
15      continue
      ...
      !set it at surface in order not calculating non-local transport 
      kinv     = kte+1
      end if
#endif
!
...
#ifdef MOIST_CU
                  rn,slimsk,im,kx,kx1,delt,dx,npmf,kinv)
#else
...
#ifdef MOIST_CU
     &         pap,paph,evap,hfx,zprecc,lndj,lq,km,km1,dt,dx, npmf,kinv)
#else
...
!
      ...
      if(pcte(j,k).gt.0.) then
        ...
        fice=1.0-fliq
#ifdef MOIST_CU
        if(pqcte(j,k) .lt. 0.)pqcte(j,k)=max(pqcte(j,k)*ztmst,-pqc(j,k))/ztmst
        if(pqite(j,k) .lt. 0.)pqite(j,k)=max(pqite(j,k)*ztmst,-pqi(j,k))/ztmst
        pqc(j,k)=pqc(j,k)+fliq*pcte(j,k)*ztmst+pqcte(j,k)*ztmst
        pqi(j,k)=pqi(j,k)+fice*pcte(j,k)*ztmst+pqite(j,k)*ztmst
#else
...
#ifdef MOIST_CU
      real     pqcen(klon,klev),       pqien(klon,klev)
      real     zqcenh(klon,klev),      zqienh(klon,klev)
      real     zmful4t(klon,klev)
      real     plende(klon,klev),      piende(klon,klev),&
     &         pcenen(klon,klev) 
      real     pqcte(klon,klev),       pqite(klon,klev)
      integer  kinv(klon)
#endif
...
#ifdef MOIST_CU
             zqumqe = pqu(jl,ikb) + plu(jl,ikb) - zqenh(jl,ikb) - zqcenh(jl,ikb)
#else
...
!----------------------------------------------------------
     ...
     &     pqcen,    zqcenh,   plende,   pqien,    zqienh,  piende,pcenen,zmful4t)
#else
     &     zqsenh,   zlglac,   lndj,     wup,      wbase,   kdpl,  pmfude_rate )
#endif
...
#ifdef MOIST_CU
          plende(jl,jk) = plende(jl,jk)*zmfs(jl)
          piende(jl,jk) = piende(jl,jk)*zmfs(jl)
          pcenen(jl,jk) = pcenen(jl,jk)*zmfs(jl)
#endif
...
!----------------------------------------------------------
     ...
     &  ,  kinv ,    pqcen,    zqcenh,   plende           &
     &  ,  zqienh,   piende,   pgeo,     pcenen,  zmful4t)
#else
     &  ,  prain,    pmfdde_rate, pmflxr, pmflxs )
#endif
...
!----------------------------------------------------------------
      call cudtdqn(klon,klev,itopm2,kctop,idtop,ldcum,loddraf, &
                 ...
                 zqcenh,zqienh,pqcte,pqite,plende,piende,pcenen,zmful4t)
#else
                 zdmfup,zdmfdp,zdpmel,ptte,pqte,pcte)
#endif
!----------------------------------------------------------------
...
#ifdef MOIST_CU
      real     pqcen(klon,klev),       pqcenh(klon,klev),&
     &         pqien(klon,klev),       pqienh(klon,klev)
#endif
      real     ptu(klon,klev),         pqu(klon,klev),&
...
! -----------------------------------------------------------
     ...
     &             cpd*pten(jl,jk)+pgeo(jl,jk))-pgeoh(jl,jk))*rcpd
        ...
        loflag(jl)=.true.
#ifdef MOIST_CU
        pqcenh(jl,jk) = pqcen(jl,jk-1)
        pqienh(jl,jk) = pqien(jl,jk-1)
#endif
      end do
     ...
     &                pgeoh(jl,klev))*rcpd
        ...
        zwmax(jl)=0.
#ifdef MOIST_CU
        pqcenh(jl,1)=pqcen(jl,1)
        pqcenh(jl,klev)=pqcen(jl,klev-1)
        pqienh(jl,1)=pqien(jl,1)
        pqienh(jl,klev)=pqien(jl,klev-1)
#endif
      end do
...
! define the variables at the first level      
      ...
      do jl=1,klon
        rho=pap(jl,klev)/ &
!#ifdef MOIST_CU
!     &         (rd*(pten(jl,klev)*(1.+vtmpc1*pqen(jl,klev)-pqcen(jl,klev)-pqien(jl,klev))))
!#else
...
!#ifdef MOIST_CU
!          vptu(jl,klev)=ptu(jl,klev)*(1.+vtmpc1*pqu(jl,klev)-plu(jl,klev))
!          vten(jl,klev)=ptenh(jl,klev)*(1.+vtmpc1*pqenh(jl,klev)-pqcenh(jl,klev)-pqienh(jl,klev))
!#else
...
#ifdef MOIST_CU
        plu(jl,jk) =(coef(jl)*(pqcenh(jl,jk+1)+pqcenh(jl,jk))&
                    +(1.-coef(jl))*plu(jl,jk+1))/(1.+coef(jl))
#endif
...
! compute the updraft speed
          vptu(jl,jk) = ptu(jl,jk)*(1.+vtmpc1*pqu(jl,jk)-plu(jl,jk))+&
                        ralfdcp*zlglac
#ifdef MOIST_CU
          vten(jl,jk) = ptenh(jl,jk)*(1.+vtmpc1*pqenh(jl,jk)-pqcenh(jl,jk)-pqienh(jl,jk))
#else
...
#ifdef MOIST_CU
        if ( plu(jl,jk)-pqcenh(jl,jk) > 0. .and. klab(jl,jk+1) == 1 ) then
#else
...
! chose nearest half level as cloud base (jk or jk+1)
              ...
              if ( zpdifftop > zpdiffbot .and. kup(jl,jk+1) > 0. ) then
                ...
                kcbot(jl) = ikb
#ifdef MOIST_CU
                plu(jl,jk+1) = max(pqcenh(jl,jk+1),1.0e-8)
#else
                plu(jl,jk+1) = 1.0e-8
#endif
...
#if (defined TUNING) || (defined MOIST_CU)
          if(kup(jl,jk) .le. 0.)then
#else
...
! define the variables at the departure level 
        if(jk .eq. levels) then
          ...
          if(loflag(jl)) then
            ...
            klab(jl,jk+1)= 1
!#ifdef MOIST_CU
!            vptu(jl,jk+1)=ptu(jl,jk+1)*(1.+vtmpc1*pqu(jl,jk+1)-plu(jl,jk+1))
!            vten(jl,jk+1)=ptenh(jl,jk+1)*(1.+vtmpc1*pqenh(jl,jk+1)-pqcenh(jl,jk+1)-pqienh(jl,jk+1))
!#else
      ...
      do jl=1,klon
        if( loflag(jl) ) then
          ...
          zlglac=zdq*((1.-foealfa(ptu(jl,jk))) - &
                      (1.-foealfa(ptu(jl,jk+1))))
#ifdef MOIST_CU
          plu(jl,jk) = pqcenh(jl,jk)+0.5*max(plu(jl,jk)-pqcenh(jl,jk),0.)
#else
...
!-----------------------------------------------------------------
     ...
     &     pqcen,    pqcenh,   plende,   pqien,    pqienh,  piende,pcenen,pmful4t) 
#else
     &     pqsenh,   plglac,   lndj,     wup,      wbase,   kdpl, pmfude_rate)
#endif
      implicit none
...
!       kcum [icum] - flag to control the call
     ...
     &         pverv(klon,klev),       pqsenh(klon,klev)
      ...
      real     pqcen(klon,klev),       pqcenh(klon,klev),  &
               pqien(klon,klev),       pqienh(klon,klev),  &
               plende(klon,klev),      piende(klon,klev),  &
               pcenen(klon,klev),      pmful4t(klon,klev)
#endif
      real     ptu(klon,klev),         pqu(klon,klev),&
...
#ifdef MOIST_CU
          pcenen(jl,jk) = pqcen(jl,jk+1)*zdmfen(jl)
#endif
...
#ifdef MOIST_CU
          zmfulk = pmful(jl,jk+1) + pcenen(jl,jk) - plude(jl,jk)
          plende(jl,jk) = pqcenh(jl,jk+1) *zdmfde(jl)
          piende(jl,jk) = pqienh(jl,jk+1) *zdmfde(jl)
#else
        ...
        do jll = 1 , jlm
          ...
          if ( pqu(jl,jk) /= zqold(jl) ) then
            ...
            zbc = ptu(jl,jk)*(1.+vtmpc1*pqu(jl,jk)-plu(jl,jk+1) - &
              zlrain(jl,jk+1))
#ifdef MOIST_CU
            zbe = ptenh(jl,jk)*(1.+vtmpc1*pqenh(jl,jk)-pqcenh(jl,jk)-pqienh(jl,jk))
#else
...
#ifdef MOIST_CU
                pqcenh(jl,jk) = max(0.5*(pqcen(jl,jk)+pqcen(jl,jk-1)),0.)
                pqienh(jl,jk) = max(0.5*(pqien(jl,jk)+pqien(jl,jk-1)),0.)
                zbuo(jl,jk) = zbc - ptenh(jl,jk)*(1.+vtmpc1*pqenh(jl,jk)-pqcenh(jl,jk)-pqienh(jl,jk))
#else
...
#ifdef MOIST_CU
     &  ,  prain,    pmfdde_rate, pmflxr, pmflxs           &
     &  ,  kinv,     pqcen,    pqcenh,   plende            &
     &  ,  pqienh,   piende,   pgeo,     pcenen,   pmful4t)
#else
     &  ,  prain,    pmfdde_rate, pmflxr, pmflxs )                                         
#endif
...
!----------------------------------------------------------------------        
     ...
     &         plglac(klon,klev)                                               
#ifdef MOIST_CU
      real     pqcenh(klon,klev),      plende(klon,klev),         &
               pqienh(klon,klev),      piende(klon,klev),         &
               pcenen(klon,klev),                                 &
               pqcen(klon,klev),       pgeo(klon,klev)
      integer  kinv(klon)
      real     pmful4t(klon,klev)
#endif
...
! local variables
      ...
      logical  llddraf
#ifdef MOIST_CU
      real     q_src(klon), s_src(klon), src
      real     xmean, xtop, xbot
...
!                  ---------------------------------                           
...
#ifdef MOIST_CU
        ikb=kcbot(jl)
        if(ldcum(jl) .and. ktype(jl) .eq. 2 .and. pmfu(jl,ikb) .gt. 1.e-8)then
           q_src(jl) = (pmfuq(jl,ikb)+pmful(jl,ikb))/pmfu(jl,ikb)
           s_src(jl) = (pmfus(jl,ikb)-foelhm(ptenh(jl,ikb))*pmful(jl,ikb))/pmfu(jl,ikb)
        end if
#endif
      enddo
...
#ifdef MOIST_CU
            pmful4t(jl,jk)=pmful(jl,jk)
            pmful(jl,jk)=pmful(jl,jk)-pmfu(jl,jk)*(pqcenh(jl,jk)+pqienh(jl,jk))
#endif
     ...
     &         foelhm(ptenh(jl,ikb))*pmful4t(jl,ikb))*zzp
          pmfuq(jl,ik)=(pmfuq(jl,ikb)+pmful4t(jl,ikb))*zzp
          pmful(jl,ik)=0.
          pmful4t(jl,ik)=0.
#else
          pmfus(jl,ik)=(pmfus(jl,ikb)-                         &
     ...
     &       (paph(jl,klev+1)-paph(jl,ikb)))
      ...
      enddo
#ifdef MOIST_CU
!Non-local transport for UW PBL
      do jl = 1, klon
        ikb=kcbot(jl)
        if(ldcum(jl) .and. ktype(jl) .eq. 2                     &
         .and. kinv(jl) .ge. kcbot(jl) .and. kinv(jl) .lt. klev .and. pmfu(jl,ikb) .gt. 1.e-8)then
         !.and.kinv(jl) .lt. klev .and. pmfu(jl,ikb) .gt. 1.e-8)then
          !qt=qv+qc
          xmean = pqen(jl,kcbot(jl))+pqcen(jl,kcbot(jl))
          xtop  = pqenh(jl,kcbot(jl))+pqcenh(jl,kcbot(jl))
          xbot  = pqenh(jl,kcbot(jl)+1)+pqcenh(jl,kcbot(jl)+1)
          src   = q_src(jl)
          call fluxbelowinv(pmfu(jl,kcbot(jl)),paph(jl,:),klev,kinv(jl),ztmst, &
                            src,xmean,xtop,xbot,pmfuq(jl,kinv(jl):klev))
          !s=cp*T+gz
          xmean = cpd*pten(jl,kcbot(jl))+pgeo(jl,kcbot(jl))
          xtop  = cpd*ptenh(jl,kcbot(jl))+pgeoh(jl,kcbot(jl))
          xbot  = cpd*ptenh(jl,kcbot(jl)+1)+pgeoh(jl,kcbot(jl)+1)
          src   = s_src(jl)
          call fluxbelowinv(pmfu(jl,kcbot(jl)),paph(jl,:),klev,kinv(jl),ztmst, &
                            src,xmean,xtop,xbot,pmfus(jl,kinv(jl):klev))
        end if
      end do
...
!--------------------------------------------------------
     subroutine cudtdqn(klon,klev,ktopm2,kctop,kdtop,ldcum, &
                     ...
                     pqcenh,pqienh,ptenqc,ptenqi,plende,piende,pcenen,pmful4t)
#else
                     pmfuq,pmfdq,pmful,pdmfup,pdmfdp,pdpmel,ptent,ptenq,pcte)
#endif
    implicit none
    ...
    real     pcte(klon,klev)
#ifdef MOIST_CU
    real     pqcenh(klon,klev), pqienh(klon,klev), ptenqc(klon,klev),  ptenqi(klon,klev)
    real     plende(klon,klev), piende(klon,klev), pcenen(klon,klev)
    real     pmful4t(klon,klev)
#endif
...
#ifdef MOIST_CU
    real     zdqtdt(klon,klev), zdqcdt(klon,klev) 
#endif
...
#ifdef MOIST_CU
               zalv*(pmful4t(jl,jk+1)-pmful4t(jl,jk)-plude(jl,jk)-pdmfup(jl,jk)-pdmfdp(jl,jk)+pcenen(jl,jk))) 
           zdqtdt(jl,jk) = zdp(jl,jk)*(zmfuq(jl,jk+1) - &
              zmfuq(jl,jk)+zmfdq(jl,jk+1)-zmfdq(jl,jk)+pmful(jl,jk+1) - &
              pmful(jl,jk)-pdmfup(jl,jk)-pdmfdp(jl,jk))
           zdqcdt(jl,jk) = zdp(jl,jk)*((pmfu(jl,jk)+pmfu(jl,jk+1))*0.5  &
              *(pqcenh(jl,jk)-pqcenh(jl,jk+1)+pqienh(jl,jk)-pqienh(jl,jk+1)) &
              +plude(jl,jk)-plende(jl,jk)-piende(jl,jk))
           zdqdt(jl,jk) = zdqtdt(jl,jk)-zdqcdt(jl,jk)
#else
...
#ifdef MOIST_CU
               zalv*(pmful4t(jl,jk)+pdmfup(jl,jk)+pdmfdp(jl,jk)))
#else
...
#ifdef MOIST_CU
         if( jk < klev ) then
         ptenqc(jl,jk) = zdp(jl,jk)*((pmfu(jl,jk)+pmfu(jl,jk+1))*0.5*(pqcenh(jl,jk)-pqcenh(jl,jk+1))-plende(jl,jk))
         ptenqi(jl,jk) = zdp(jl,jk)*((pmfu(jl,jk)+pmfu(jl,jk+1))*0.5*(pqienh(jl,jk)-pqienh(jl,jk+1))-piende(jl,jk))
         else
         ptenqc(jl,jk) = 0.
         ptenqi(jl,jk) = 0.
         end if
...
!---------------------------------------------------------
...
#ifdef MOIST_CU
    subroutine fluxbelowinv(cbmf, ps0, klev, kinv, dt, xsrc, xmean,xtopin,xbotin,xflx)
    integer klev, kinv
    real    cbmf, dt, xsrc, xmean, xtopin, xbotin
    real    ps0(1:klev+1)
    real    xflx(kinv:klev)
    ...
    rpeff = min( max(0.,rpeff), 1. )
    if( rpeff .eq. 0. .or. rpeff .eq. 1. ) then
        xbot = xmean
        xtop = xmean
    end if
    ! Below two commented-out lines are the old code replacing the above 'if'
    ! block.
    ...
    ! ----------------------------------------------------------------------- !
    do k = kinv+1, klev
        xflx(k) = cbmf * ( xsrc - xbot ) * ( ps0(klev+1) - ps0(k) ) / ( ps0(klev+1) - pinv )
    end do
    if( rr .le. 1. ) then
        xflx(kinv+1) = xflx(kinv)-( 1.-rr )*cbmf*(xtop_ori-xbot_ori)
    end if
...
end module module_cu_mtiedtke_v381
