...
module module_cu_ntiedtke_v381
     ...
     use grist_PhysW_nml_module, only: para1_zoentr_test, para2_zoentr_test, para1_eta_test, para2_eta_test
...
#ifdef ZOENTR_TEST
            zoentr(jl) = para1_zoentr_test*zoentr(jl)
#endif
          end if
          if ( jk < kcbot(jl) ) then
            zmfmax = (paph(jl,jk)-paph(jl,jk-1))*zcons2
...
#ifdef ZOENTR_TEST
                zoentr(jl) = para2_zoentr_test*zoentr(jl)
#endif
              else
                zoentr(jl) = 0.
              end if
...
#ifdef ZOENTR_TEST
            pdmfde(jl) = para1_zoentr_test*0.75e-4*zmf
#endif
          end if
        end if
      end do
